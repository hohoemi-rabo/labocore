...
- [ ] **実機で絵文字（📢 お知らせ / 📅 今月のよてい / 📋 ✅ コピーボタン / 😊 ブランドマーク）が出るか確認する。** 開発環境（WSL）には絵文字フォントが無く豆腐になるため、20 では見た目を確認できていない
- [ ] **実機でプロンプトのコピーボタンを確認する。** `navigator.clipboard` は secure context 限定で、`http://192.168.x.x:3000` では **undefined**（実測確認済み）。20 で `textarea` + `execCommand` のフォールバックを入れてあるが、実際にコピーできるかは実機でしか分からない。ダメなら ✅ ではなく「コピーできませんでした…」が出る
  - **⚠️ LAN の IP で確認するときは `npm run dev` を使う。** 本番ビルド（`npm start`）だと合言葉 Cookie が `Secure` 付きになり、平文 http の IP アクセスではブラウザに保存されず合言葉画面から進めなくなる（`secure: NODE_ENV === "production"`）
- [ ] **水曜午後クラスを足すとクラスタブが6個になり横スクロールが発生する。** 375px でアクティブなタブが画面外から始まらないか（クライアント JS を持たないので自動スクロールはしない）を確認する
