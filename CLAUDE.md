...
実装は docs/ のチケット順に進行中。**01〜06（基盤・デザイントークン / DBスキーマ・RLS / 認証 / 共通レイアウト / コマ管理 / 生徒管理）まで完了**、07〜12 は未着手。既存コードで確立済みの構成・規約は「ディレクトリ構成と実装パターン」を参照し、新規実装もそれに合わせる。
...
- `src/lib/format.ts` — `WEEKDAY_LABELS` / `formatTimeRange` / `formatYen`。
