...
実装は docs/ のチケット順に進行中。**01〜11（基盤・デザイントークン / DBスキーマ・RLS / 認証 / 共通レイアウト / コマ管理 / 生徒管理 / 生徒詳細 / 休講日管理 / 今日の出欠 / カレンダー / 月次集計）まで完了**。12（仕上げ・デプロイ）はコード側の品質確認・未使用コード削除・README 整備まで完了、Vercel デプロイと実機確認は運用者作業として残っている。既存コードで確立済みの構成・規約は「ディレクトリ構成と実装パターン」を参照し、新規実装もそれに合わせる。
...
- `src/lib/format.ts` — `WEEKDAY_LABELS` / `formatTimeRange` / `formatYen` / `formatMonthJa` / `formatDateJa`。
- `src/components/yen.tsx` — `<Yen amount>`。プロミネントな金額表示用（`¥` を一回り小さく muted・数字は `tabular-nums`）。素の文字列が要る箇所は `formatYen`。
...
- `src/components/page-placeholder.tsx` — 未実装画面のプレースホルダ。
